...
class DT:
    ...
    def insert_node(self, X,Y,depth = 0):
        ...
        node.children['right'] = self.insert_node(X[right_idxs], Y[right_idxs],depth+1)
        return node
    ...
    def Los_Gimmis(self,X_column, y):
            ...
            for punto_medio in adyacent_average:
                ...
                #print("gini_der")
                #print(gini_der)
                ponderado = (len(X_izq) / len(X_column)) * gini_izq + (len(X_der) / len(X_column)) * gini_der
                #print("ponderado")
                #print(ponderado)
...
# Predecir
y_pred = model.predict_2(X_test)

print("Accuracy: ", accuracy_score(y_test, y_pred))

# experimentar 100 veces
'''
accs = []
...
print("Accuracy promedio: ", np.mean(accs))